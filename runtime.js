(() => {
  ...
  const NANO_API_BASE='https://nano-gpt.com/api/v1';
  const NANO_IMAGE_BASE='https://nano-gpt.com/v1';
  // NanoGPT integration: one authoritative connection flow.
  // The model catalogue request is the connection test. If direct browser CORS is unavailable,
  // Aurora retries through a compatibility relay. There is deliberately NO second background
  // connection request, so a successful catalogue load cannot be followed by a stuck spinner.
  function loadNanoGPTSettings(){
    try{
      const raw=JSON.parse(localStorage.getItem('aurora_nanogpt_settings_v3')||localStorage.getItem('aurora_nanogpt_settings_v2')||localStorage.getItem('aurora_nanogpt_settings_v1')||'null');
      if(raw&&typeof raw==='object') return raw;
    }catch(e){}
    return {};
  }
  const NANO_DEFAULTS={
    access:'ask',maxRequests:0,requestCount:0,provider:'NanoGPT',endpoint:NANO_API_BASE,apiKey:'',
  ...
  function normalizeNanoGPT(raw){
    ...
    n.relayEnabled=n.relayEnabled!==false;
    // Busy flags are transient UI state and must NEVER survive a reload/render cycle.
    n.connectionBusy=false; n.textBusy=false; n.imageBusy=false;
    return n;
  }
  function saveNanoGPTSettings(n){try{return localStorage.setItem('aurora_nanogpt_settings_v3',JSON.stringify(normalizeNanoGPT(n))),true}catch(e){return false}}
  function nanoDraft(){if(!state.settingsDraft)beginSettings();state.settingsDraft.nano=normalizeNanoGPT(state.settingsDraft.nano);return state.settingsDraft.nano}
  function nanoHeaders(n,method='GET',mode='bearer'){
    const h={Accept:'application/json'};
    if(mode!=='none'&&String(n.apiKey||'').trim())h.Authorization='Bearer '+String(n.apiKey).trim();
  ...
  async function nanoOneFetch(url,n,options={}){
    const method=String(options.method||'GET').toUpperCase();
    const timeoutMs=Math.max(4000,Number(options.timeoutMs)||12000);
    const controller=new AbortController();
    const timer=setTimeout(()=>controller.abort(),timeoutMs);
    try{
      const response=await fetch(url,{method,headers:options.headers||nanoHeaders(n,method,options.authMode||'bearer'),body:options.body,cache:'no-store',credentials:'omit',mode:'cors',signal:controller.signal});
      ...
      if(!response.ok){
        ...
        const err=new Error('HTTP '+response.status+' — '+String(msg).slice(0,300));err.httpStatus=response.status;err.apiError=data?.error||null;throw err;
      }
      return data;
    }catch(e){
      if(e?.name==='AbortError'){const err=new Error('Request timed out after '+Math.round(timeoutMs/1000)+' seconds.');err.code='TIMEOUT';throw err}
      throw e;
    }finally{clearTimeout(timer)}
  }
  async function nanoFetchJson(url,n,options={}){
    // Direct NanoGPT request. No third-party relay is used.
    return await nanoOneFetch(url,n,options);
  }
  async function nanoFetchCatalog(url,n){
    // NanoGPT documents model listings as public GET endpoints.
    // Deliberately omit the API key and custom auth header to avoid browser preflight.
    return await nanoOneFetch(url,n,{method:'GET',authMode:'none',headers:{Accept:'application/json'},timeoutMs:12000});
  }
  function nanoErrorText(err){
    if(err?.code==='TIMEOUT')return err.message;
    if(err?.httpStatus===400)return String(err.message||'NanoGPT rejected the request (HTTP 400).');
    if(err?.httpStatus===401)return 'NanoGPT rejected the API key (HTTP 401).';
    if(err?.httpStatus===403)return 'NanoGPT rejected the API key or browser origin (HTTP 403).';
  ...
  async function nanoLoadImageModels(){
    const n=syncNanoDraftFromDom();if(n.imageBusy)return false;
    n.imageBusy=true;n.imageStatus='Loading live NanoGPT image catalog…';render();
    try{
      const data=await nanoFetchCatalog(NANO_API_BASE+'/image-models?detailed=false',n);
      const models=nanoCatalogImageModels(data);if(!models.length)throw new Error('NanoGPT returned no image models.');
      n.imageModels=models;if(!n.imageModel||!models.some(x=>x.id===n.imageModel))n.imageModel=models[0].id;
      n.imageStatus=`${models.length} live image models loaded.`;saveNanoGPTSettings(n);state.nanoGPT=normalizeNanoGPT(n);toast(`Loaded ${models.length} image models`);return true;
    }catch(err){n.imageStatus='Could not load the live image catalog · '+nanoErrorText(err);toast('Image model loading failed');return false}
    finally{n.imageBusy=false;render()}
  }
  function syncNanoDraftFromDom(){const n=nanoDraft();const key=document.getElementById('nano-api-key');if(key)n.apiKey=String(key.value||'').trim();return n}
  function nanoCanRequest(n){if(n.access==='off')return{ok:false,reason:'API access is disabled.'};if(n.maxRequests>0&&n.requestCount>=n.maxRequests)return{ok:false,reason:'Maximum AI request limit reached.'};if(!n.apiKey.trim())return{ok:false,reason:'Add your NanoGPT API key first.'};return{ok:true}}
  ...
  function more(){
    ...
    return layout(`<div class="page"><div class="backrow"><button class="back" data-action="back">‹ Back</button></div>
      ...
      <section class="section" style="margin-top:18px"><div class="card nano-card"><div class="eyebrow">CONNECTED AI PROVIDER</div><h2>NanoGPT</h2>
        <div class="nano-note">Aurora uses NanoGPT’s documented OpenAI-compatible API. The model catalogue is a public GET request, so Aurora loads it directly without sending your API key. Your API key is used only for actual AI requests.</div>
        ...
        <div class="nano-model-actions"><button class="primary" data-nano-test ${n.connectionBusy?'disabled':''}>${n.connectionBusy?'Connecting…':'Connect &amp; Load Models'}</button></div>
        <div class="nano-status ${n.connectionState==='ok'?'ok':n.connectionState==='error'?'error':''}"><b>NanoGPT</b><br>${safeText(n.connectionStatus)}${n.balanceUsd!=null?' · Balance: $'+safeText(n.balanceUsd):''}</div>
      </div></section>
