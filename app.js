  ...
  function nanoHeaders(n,accept,method){const m=String(method||'GET').toUpperCase();const h={};if(String(n.apiKey||'').trim())h.Authorization='Bearer '+String(n.apiKey||'').trim();if(accept)h.Accept=accept;if(m!=='GET'&&m!=='HEAD'&&m!=='OPTIONS')h['Content-Type']='application/json';return h}
  ...
  function nanoDirectHeaders(n,accept,method){
  ...
  function nanoWithHardTimeout(p,ms,label){
    return Promise.race([p,new Promise((_,reject)=>setTimeout(()=>{const e=new Error((label||'NanoGPT')+' timed out after '+ms/1000+' seconds.');e.code='TIMEOUT';reject(e)},ms))]);
  }
  async function nanoFetchJson(url,n,options={}){
    ...
    const headers=Object.assign(nanoDirectHeaders(auth?n:{apiKey:''},'application/json',method),options.headers||{});
    delete requestOptions.signal;
    requestOptions.headers=headers;
    requestOptions.mode='cors'; requestOptions.credentials='omit'; requestOptions.cache='no-store';
    const response=await nanoWithHardTimeout(fetch(url,requestOptions),7000,'NanoGPT request');
    let raw='';
    raw=await nanoWithHardTimeout(response.text(),7000,'NanoGPT response');
    let data=null; try{data=raw?JSON.parse(raw):null}catch(e){data={raw:raw}};
    if(!response.ok){
      const msg=data?.error?.message||data?.message||(typeof data?.error==='string'?data.error:'')||('HTTP '+response.status);
      const err=new Error('HTTP '+response.status+' — '+String(msg).slice(0,300)); err.httpStatus=response.status; err.requestId=response.headers?.get?.('x-request-id')||''; throw err;
    }
    return data;
  }
  function nanoJinaUrl(url){ return 'https://r.jina.ai/'+url; }
  async function nanoFetchCatalogWithFallback(url,n,kind){
    try{
      return await nanoFetchJson(url,n,{auth:false});
    }catch(firstErr){
      if(firstErr?.code!=='NETWORK'&&firstErr?.code!=='TIMEOUT') throw firstErr;
      try{
        const proxy=await nanoWithHardTimeout(fetch(nanoJinaUrl(url),{method:'GET',headers:{Accept:'application/json'},mode:'cors',credentials:'omit',cache:'no-store'}),7000,'Catalog relay');
        const raw=await nanoWithHardTimeout(proxy.text(),7000,'Catalog relay response');
        let data=null; try{data=raw?JSON.parse(raw):null}catch(e){data={raw:raw}};
        if(!proxy.ok) throw new Error('Relay HTTP '+proxy.status);
        if(data&&Array.isArray(data.data)) return data;
        throw new Error('Relay returned an unexpected '+kind+' catalog format.');
      }catch(relayErr){
        const e=new Error('Direct browser request failed and catalog relay also failed. '+(firstErr?.message||'')+' '+(relayErr?.message||''));
        e.code='CATALOG_UNAVAILABLE'; throw e;
      }
    }
  }
  function nanoErrorText(err){
    if(err?.code==='TIMEOUT') return String(err.message||'NanoGPT request timed out.');
    if(err?.code==='NETWORK') return 'Browser could not reach NanoGPT (CORS/network/origin restriction).';
    if(err?.code==='CATALOG_UNAVAILABLE') return String(err.message||'Catalog unavailable in this local browser environment.');
    if(err?.httpStatus===403) return 'NanoGPT rejected the browser origin/key (HTTP 403). Check the API key Allowed browser origins.';
    if(err?.httpStatus===401) return 'NanoGPT rejected the API key (HTTP 401).';
    return String(err?.message||err||'Unknown error');
  }
  function nanoErrorText(err){
    if(err?.code==='TIMEOUT') return 'Timed out after 15s — NanoGPT did not answer.';
    if(err?.code==='NETWORK') return 'Browser could not reach NanoGPT — possible network/CORS block.';
    return String(err?.message||err||'Unknown error');
  }
  ...
  async function nanoTestConnection(){
    ...
    if(!n.apiKey.trim()){nanoSetConnection(n,'Not connected · enter your API key.','error');render();toast('Add your NanoGPT API key first');return false}
    if(n.connectionBusy) return false;
    n.connectionBusy=true; nanoSetConnection(n,'Checking NanoGPT reachability…','loading'); render();
    try{
      // Authenticated model-list request is the diagnostic: NanoGPT documents Bearer auth on /api/v1/models.
      const authData=await nanoFetchJson(nanoEndpoint(n,'/models'),n,{auth:true});
      ...
      n.connectionBusy=false; render(); toast('NanoGPT connection successful');
      return true;
    }catch(err){
      n.connectionBusy=false;
      const msg=nanoErrorText(err);
      nanoSetConnection(n,'NanoGPT could not be reached · '+msg,'error');
      render(); toast('NanoGPT connection failed'); return false;
    }
  }
  ...
  function more(){
    ...
    return layout(`<div class="page"><div class="backrow"><button class="back" data-action="back">‹ Back</button></div>
      ...
      <section class="section" style="margin-top:18px"><div class="card nano-card"><div class="eyebrow">CONNECTED AI PROVIDER</div><h2>NanoGPT</h2>
        ...
        <div class="nano-model-actions"><button class="primary" data-nano-test ${n.connectionBusy?'disabled':''}>${n.connectionBusy?'Testing…':'Test Connection'}</button></div>
